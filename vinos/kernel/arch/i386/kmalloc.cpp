...
static uint32_t mem_beginning;
static uint32_t end_of_kernel_mem;
static kmalloc_meta* last_header_location;

bool kmalloc::enabled = false;

void kernel::enable_kmalloc(uintptr_t placement, size_t size) {
    placement_address = placement;
    mem_beginning = placement;
    last_header_location = (kmalloc_meta*)mem_beginning;
    end_of_kernel_mem = placement_address + size;
    kmalloc::enabled = true;
}
...
void kernel::kfree(void* ptr) {
    if(!kmalloc::enabled) { kernel::panic::panic("kmalloc disabled", "", true); }
